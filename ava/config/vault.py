...
class VaultClient:
    ...
    def __init__(self, config: Optional[VaultConfig] = None):
        ...
        if not HVAC_AVAILABLE:
            raise ImportError("hvac library not installed. Install with: pip install hvac")
    ...
    def connect(self) -> bool:
        ...
        try:
            # Client erstellen
            self.client = hvac.Client(url=self.config.addr, namespace=self.config.namespace)
    ...
    def get_secret(self, path: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            secret = self.client.secrets.kv.v2.read_secret_version(
                path=full_path, mount_point=self.config.mount
            )
