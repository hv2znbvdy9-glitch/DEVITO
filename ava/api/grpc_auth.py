...
class APIKeyStore:
    ...
    def _load_default_keys(self):
        ...
        if admin_key:
            self.create_key(key=admin_key, role=Role.ADMIN, owner="system", expires_at=None)
...
class AdvancedAuthInterceptor(aio.ServerInterceptor):
    ...
    async def intercept_service(self, continuation, handler_call_details):
        ...
        if not role:
            ...
            return grpc.unary_unary_rpc_method_handler(
                lambda req, ctx: ctx.abort(grpc.StatusCode.UNAUTHENTICATED, "Authentication failed")
            )
    ...
    def _check_rate_limit(self, user_id: str) -> bool:
        ...
        self.request_counts[user_id] = [
            ts for ts in self.request_counts[user_id] if now - ts < self.rate_window
        ]
