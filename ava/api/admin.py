...
async def admin_login(request: AdminLoginRequest):
    ...
    if request.username != OWNER_USERNAME:
        threat_log.log_threat(
            "unknown", "/api/admin/login", ThreatLevel.WARNING, f"Invalid login: {request.username}"
        )
    ...
    audit_logger.log_action(OWNER_USERNAME, "LOGIN", "admin_console", "SUCCESS")
...
async def get_threats(request: Request, api_key: Optional[str] = Header(None)):
    ...
    audit_logger.log_action(OWNER_USERNAME, "VIEW", "threat_report", "SUCCESS")

    return {"timestamp": datetime.now().isoformat(), "report": report}


@admin_router.post("/block-ip")
async def block_ip(
    request: Request, block_request: IPBlockRequest, api_key: Optional[str] = Header(None)
):
...
async def get_blocked_ips(request: Request, api_key: Optional[str] = Header(None)):
    ...
    blocked_ips = [
        {"ip": ip, "blocked_until": expires.isoformat()}
...
async def list_api_keys(request: Request, api_key: Optional[str] = Header(None)):
    ...
    audit_logger.log_action(OWNER_USERNAME, "VIEW", "api_keys", "SUCCESS")
...
async def revoke_api_key(
    request: Request, api_key_to_revoke: str, api_key: Optional[str] = Header(None)
):
    ...
    audit_logger.log_action(OWNER_USERNAME, "REVOKE_API_KEY", api_key_to_revoke[:10], "SUCCESS")
...
async def get_audit_log(request: Request, limit: int = 100, api_key: Optional[str] = Header(None)):
    """Zeige Audit-Log"""
    ...
    return {"count": len(logs), "logs": logs, "timestamp": datetime.now().isoformat()}
...
async def update_rate_limit(
    request: Request, config: RateLimitRequest, api_key: Optional[str] = Header(None)
):
