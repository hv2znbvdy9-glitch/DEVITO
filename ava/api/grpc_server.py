...
class AuthInterceptor(grpc.aio.ServerInterceptor):
    ...
    async def intercept_service(self, continuation, handler_call_details):
        ...
        if not auth_header.startswith(BEARER_PREFIX):
            logger.warning("🚨 Unauthorized access attempt: Missing/Invalid Bearer token")
...
class SecureGRPCServer:
    ...
    def _load_credentials(self) -> grpc.ServerCredentials:
        ...
        if not all(os.path.exists(f) for f in [SERVER_CERT, SERVER_KEY, CA_CERT]):
            logger.warning(
                "⚠️  Missing TLS certificates! Falling back to INSECURE mode (DEV ONLY!)"
            )
    ...
    async def start(self):
        ...
        AVAServiceImplementation(self.db_pool, self.sync_queue, self.task_scheduler)
