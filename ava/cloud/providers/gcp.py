...
class GCPProvider(CloudSync):
        ...
        try:
            if self.credentials_path:
                self.storage_client = storage.Client(
                    project=self.project_id, credentials=self._load_credentials()
                )
            else:
    ...
    async def upload_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            blob.upload_from_string(json_data.encode("utf-8"), content_type="application/json")
    ...
    async def download_data(self) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            blobs = list(self.storage_client.list_blobs(self.bucket_name, max_results=1))
