...
class ZeroTrustIoTAuthenticator:
    ...
    def issue_credentials(
        self, device_id: str, permissions: List[str], validity_hours: int = 24
    ) -> DeviceCredentials:
    ...
    def verify_request(
        self, api_key: str, signature: str, request_data: str
        ...
        expected_signature = hmac.new(
            creds.secret_key.encode(), request_data.encode(), hashlib.sha256
        ).hexdigest()
        ...
        recent_failures = [t for t in self.auth_attempts[api_key] if time.time() - t < 60]
    ...
    def rotate_credentials(self, api_key: str) -> Optional[DeviceCredentials]:
        ...
        new_creds = self.issue_credentials(
            old_creds.device_id, old_creds.permissions, validity_hours=24
        )
...
class FirmwareIntegrityMonitor:
    ...
    def register_firmware_baseline(self, device_id: str, firmware_data: bytes) -> str:
    ...
    def verify_firmware_integrity(
        self, device_id: str, current_firmware: bytes
    ) -> Tuple[bool, Optional[str]]:
        ...
        if current_hash != expected_hash:
            ...
            print("🚨 FIRMWARE INTEGRITY VIOLATION!")
...
class DeviceBehaviorAnalyzer:
    ...
    def analyze_behavior(self, device_id: str, metrics: Dict) -> Tuple[bool, List[str]]:
        ...
        if "network_traffic" in metrics:
            ...
            if not (
                profile.normal_network_traffic[0] <= traffic <= profile.normal_network_traffic[1]
            ):
        ...
        if "requests_per_minute" in metrics:
            ...
            if not (
                profile.baseline_requests_per_minute[0]
                <= rpm
                <= profile.baseline_requests_per_minute[1]
            ):
...
class AutomatedIoTThreatResponse:
    ...
    def __init__(
        self,
        authenticator: ZeroTrustIoTAuthenticator,
    ...
    def respond_to_threat(
        self, device_id: str, threat_type: str, severity: str, api_key: Optional[str] = None
    ):
...
if __name__ == "__main__":
    ...
    creds = auth.issue_credentials(
        device_id="camera_001", permissions=["read_sensors", "write_logs"]
    )
    ...
    # Normal behavior
    is_normal, anomalies = behavior.analyze_behavior(
        "camera_001", {"cpu_usage": 15.0, "memory_usage": 30.0, "network_traffic": 500.0}
    )
