...
class TangleSecurityNetwork:
    ...
    def add_threat_transaction(self, threat_data: Dict) -> Transaction:
        ...
        tx_id = hashlib.sha256(f"{time.time()}:{json.dumps(threat_data)}".encode()).hexdigest()[:16]
        ...
        print("💠 Threat TX added to Tangle")
        print(f"   TX ID: {tx_id}")
    ...
    def get_tangle_statistics(self) -> Dict:
        ...
        return {
            ...
            "average_weight": (
                sum(tx.cumulative_weight for tx in self.transactions.values())
    ...
    def visualize_tangle(self, filename: str = "tangle_visualization.json"):
        ...
        for tx_id, tx in self.transactions.items():
            ...
            for approved_id in tx.approves:
                edges.append({"from": tx_id, "to": approved_id})

        tangle_data = {"nodes": nodes, "edges": edges, "statistics": self.get_tangle_statistics()}
...
class IoTSecurityManager:
    ...
    def register_device(
        self, device_type: str, ip_address: str, firmware_version: str = "1.0.0"
    ) -> IoTDevice:
    ...
    def get_device_statistics(self) -> Dict:
        ...
        return {
            ...
            "average_security_score": (
                sum(d.security_score for d in self.devices.values()) / len(self.devices)
                if self.devices
                else 0
            ),
            "devices_by_type": {},
...
if __name__ == "__main__":
    ...
    iot_mgr.report_iot_threat(
        camera1.device_id, {"attack_type": "unauthorized_access", "severity": "HIGH"}
    )
