...
class AccessPolicy:
    ...
    def check_compliance(self, context: AccessContext) -> Tuple[bool, List[str]]:
        ...
        if context.user.trust_score < self.required_trust_score:
            violations.append(f"User trust score too low: {context.user.trust_score} < {self.required_trust_score}")
        if context.device.trust_score < self.required_trust_score:
            violations.append(f"Device trust score too low: {context.device.trust_score} < {self.required_trust_score}")
...
class ZeroTrustEngine:
    ...
    def verify_access(self, context: AccessContext) -> Tuple[bool, str, List[str]]:
        ...
        # Calculate risk
        risk = context.calculate_risk()
        
        # Find applicable policies
        applicable_policies = [
            policy for policy in self.policies.values()
            if policy.matches_resource(context.resource)
        ]
    ...
    def get_access_statistics(self) -> Dict[str, Any]:
        ...
        avg_device_trust = sum(log["device_trust"] for log in self.access_log) / max(total_requests, 1)
