...
class Incident:
    ...
    def add_timeline_event(self, event: str, details: Optional[Dict] = None):
        ...
        self.timeline.append(
            {"timestamp": datetime.utcnow().isoformat(), "event": event, "details": details or {}}
        )
...
class ResponseExecutor:
    ...
    async def execute(
        self, action: ResponseAction, incident: Incident, params: Optional[Dict[str, Any]] = None
    ) -> Tuple[bool, str]:
        """
    ...
    def _capture_system_state(self) -> Dict[str, Any]:
        ...
        return {
            "processes": [
                {"pid": p.pid, "name": p.name()} for p in psutil.process_iter(["pid", "name"])
            ],
...
class IncidentResponseSystem:
    ...
    async def respond_to_incident(self, incident: Incident):
        ...
        applicable_playbooks = [
            playbook for playbook in self.playbooks.values() if playbook.should_activate(incident)
        ]
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        incident = Incident(
            ...
            metadata={
                "category": "malware",
                "action_params": {"pid": 12345, "file_path": "/tmp/suspicious.exe"},
            },
        )
