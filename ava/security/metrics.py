...
try:
    from prometheus_client import Counter, Gauge, Histogram, Summary, Info, CollectorRegistry, generate_latest
...
class SecurityMetrics:
    ...
    def __init__(self, registry: Optional['CollectorRegistry'] = None):
        ...
        self.cookie_scans_total = Counter(
            'ava_security_cookie_scans_total',
            'Total number of cookies scanned',
            registry=registry
        )
...
def get_metrics(registry: Optional['CollectorRegistry'] = None) -> SecurityMetrics:
    ...
    if _metrics is None:
        ...
        _metrics.set_system_info(
            version="4.0.0",
            platform="linux"  # Could be detected
        )
    return _metrics
