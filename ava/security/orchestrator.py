...
try:
    from .threat_intelligence import get_threat_intelligence, ThreatLevel
    from .zero_trust import ZeroTrustEngine
    from .incident_response import get_incident_response_system, IncidentSeverity
...
class SecurityOrchestrator:
    ...
    def get_security_score(self) -> int:
        ...
        if self.incident_response:
            ir_stats = self.incident_response.get_statistics()
            open_incidents = sum(
                count for status, count in ir_stats["by_status"].items()
...
[blue]Registered Devices:[/blue] {zt_stats['total_devices']}
    ...
    def print_status_report(self):
        ...
        for component, info in status["components"].items():
            ...
            self.console.print(f"  {status_icon} {component.replace('_', ' ').title()}: {info['status']}")
        ...
        if self.threat_intel:
            ...
            self.console.print(f"  MITRE Techniques Detected: {len(ti_report['detected_mitre_techniques'])}\n")
