...
class CookieSecurityScanner:
    ...
    TRACKING_COOKIES = {
        ...
        "_fbp", "fr",  # Facebook
        "DSID", "IDE", "NID",  # DoubleClick
        "_ym_", "yandexuid",  # Yandex
        "uuid", "uuid2",  # Third-party tracking
    }
    ...
    def load_state(self):
        ...
        if state_file.exists():
            try:
                ...
                logger.info(f"✅ Cookie scanner state loaded: {len(self.learned_malicious_patterns)} patterns, "
    ...
    def scan_cookie(self, cookie: Cookie, request_domain: Optional[str] = None) -> List[CookieThreat]:
        """
    ...
    def _check_suspicious_encoding(self, cookie: Cookie) -> Optional[CookieThreat]:
        ...
        try:
            if len(cookie.value) > 20:
                ...
                if any(pattern in decoded_b64.lower() for pattern in ['<script', 'javascript:', 'eval(']):
                    return CookieThreat(
                        threat_type=CookieThreatType.SUSPICIOUS_ENCODING,
                        severity=8,
                        cookie_name=cookie.name,
                        cookie_value=cookie.value,
                        description=f"Base64-encoded malicious content detected",
                ...
                for i in range(len(decoded_value) - 5):
                    ...
                    if pattern not in self.learned_malicious_patterns:
                        ...
                        if len(self.learned_malicious_patterns) > 1000:
                            self.learned_malicious_patterns = self.learned_malicious_patterns[-1000:]
...
⚠️ THREAT DISTRIBUTION:"""
        ...
        if self.threats_detected:
            ...
            for threat in self.threats_detected[-10:]:
                report += f"  - [{threat.severity}/10] {threat.threat_type.value}: {threat.cookie_name}\n"
