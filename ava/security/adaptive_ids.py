...
class SelfLearningEngine:
    ...
    def learn_from_attack(self, attacker_fp: NetworkFingerprint, 
                         attack_type: str, indicators: List[str]) -> AttackPattern:
        ...
        logger.warning(f"🧠 LEARNED NEW ATTACK PATTERN: {pattern.name} from {attacker_fp.ip_address}")
    ...
    def update_behavior_profile(self, fingerprint: NetworkFingerprint):
        ...
        features = [
            fingerprint.request_count / max(1, (datetime.utcnow() - fingerprint.first_seen).seconds),
    ...
    def detect_anomaly(self, fingerprint: NetworkFingerprint) -> float:
        ...
        if profile_key not in self.behavior_profiles or len(self.behavior_profiles[profile_key]) < 10:
...
class AdaptiveNetworkIDS:
    ...
    def load_state(self):
        ...
        if state_file.exists():
            try:
                ...
                logger.info(f"✅ State loaded: {len(self.fingerprints)} fingerprints, "
    ...
    def get_or_create_fingerprint(self, ip: str, mac: Optional[str] = None) -> NetworkFingerprint:
        ...
        if key not in self.fingerprints:
            self.fingerprints[key] = NetworkFingerprint(
                ip_address=ip,
                mac_address=mac
            )
    ...
    async def scan_address(self, ip: str, mac: Optional[str] = None, 
                          ports: Optional[List[int]] = None,
                          user_agent: Optional[str] = None) -> Tuple[bool, str, ThreatLevel]:
        """
        ...
        if mac:
            if self.mac_analyzer.is_suspicious(mac):
                ...
                logger.warning(f"⚠️ Suspicious MAC detected: {mac} ({self.mac_analyzer.get_vendor(mac)})")
        ...
        if anomaly_score > 70:
            ...
            pattern = self.learning_engine.learn_from_attack(fp, "anomaly", indicators)
            ...
            if anomaly_score > 90:
                ...
                return False, f"BLOCKED: Critical anomaly detected ({anomaly_score:.1f})", ThreatLevel.CRITICAL
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        allowed, reason, threat = await anids.scan_address(
            ip="192.168.1.101",
            mac="00:00:00:00:00:00",  # Null MAC
            ports=[22, 23, 3389]
        )
