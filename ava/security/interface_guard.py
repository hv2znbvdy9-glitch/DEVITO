...
class InterfaceGuard:
    ...
    def register_interface(self, interface_type: InterfaceType, host: str, port: int,
                          path: str = "/", description: str = "",
                          requires_auth: bool = True, whitelist_only: bool = True,
                          max_connections: int = 100, rate_limit: int = 10) -> str:
    ...
    def check_access(self, interface_id: str, source_ip: str, source_port: int = 0,
                    payload: str = "", threat_score: float = 0.0) -> Dict:
        """
        ...
        if interface.whitelist_only:
            if source_ip not in self.global_whitelist:
                ...
                attempt = self._log_attempt(
                    interface_id, source_ip, source_port, payload,
                    ThreatLevel.ANNIHILATE, "NOT_WHITELISTED"
                )
                
                logger.error(f"💥 ANNIHILATE: {source_ip} → {interface.interface_type.value}:{interface.port}")
                logger.error(f"   Reason: NOT IN WHITELIST (FOREIGN ACCESS)")
        ...
        if threat_level == ThreatLevel.ANNIHILATE:
            ...
            attempt = self._log_attempt(
                interface_id, source_ip, source_port, payload,
                ThreatLevel.ANNIHILATE, "INSTANT_ANNIHILATION"
            )
        ...
        if interface.requires_auth:
            ...
            if "auth" not in payload.lower() and "token" not in payload.lower():
                ...
                attempt = self._log_attempt(
                    interface_id, source_ip, source_port, payload,
                    ThreatLevel.BLOCK, "NO_AUTHENTICATION"
                )
    ...
    def _log_attempt(self, interface_id: str, source_ip: str, source_port: int,
                    payload: str, threat_level: ThreatLevel, action: str) -> AccessAttempt:
...
if __name__ == "__main__":
    ...
    http_id = guard.register_interface(
        ...
        description="Main HTTP API", whitelist_only=True
    )
    
    ws_id = guard.register_interface(
        InterfaceType.WEBSOCKET, "0.0.0.0", 8081,
        description="WebSocket Real-time", whitelist_only=True
    )
    
    db_id = guard.register_interface(
        InterfaceType.DATABASE, "localhost", 5432,
        description="PostgreSQL Database", whitelist_only=True
    )
