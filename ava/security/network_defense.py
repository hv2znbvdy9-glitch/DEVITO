...
class TrafficAnalyzer:
    ...
    def __init__(self):
        self.traffic_stats: Dict[str, Dict[str, Any]] = defaultdict(
            lambda: {
                "packet_count": 0,
                "bytes_sent": 0,
                "bytes_received": 0,
    ...
    def get_top_talkers(self, limit: int = 10) -> List[Tuple[str, int]]:
        ...
        talkers = [(ip, stats["bytes_sent"]) for ip, stats in self.traffic_stats.items()]
...
class NetworkDefenseEngine:
    ...
    def _create_alert(
        ...
        packet: Optional[NetworkPacket] = None,
    ):
