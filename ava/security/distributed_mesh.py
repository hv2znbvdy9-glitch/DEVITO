...
class SecurityNode:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            age < 60 and  # Gesehen in letzten 60s
            self.state != SecurityState.ISOLATED and
            self.trust_score > 30 and
            self.load < 0.9
        )
...
class DistributedSecurityMesh:
    ...
    def load_state(self):
        ...
        if state_file.exists():
            try:
                ...
                logger.info(f"✅ Mesh state loaded: {len(self.nodes)} nodes, "
    ...
    def register_node(self, node: SecurityNode):
        ...
        logger.info(f"➕ Node registered: {node.node_id} ({node.node_type.value}) @ {node.ip_address}")
    ...
    def _propagate_event(self, event: SecurityEvent):
        ...
        logger.warning(f"🔄 Propagating critical event: {event.event_type} (Severity: {event.severity})")
    ...
    def update_node_health(self):
        ...
        for node_id, node in self.nodes.items():
            if not node.is_healthy():
                ...
                logger.warning(f"⚠️ Unhealthy node: {node_id} (State: {node.state.value}, "
