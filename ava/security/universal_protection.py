...
class HTTPProtector(InterfaceProtector):
    ...
    async def protect(self, request: InterfaceRequest) -> InterfaceResponse:
        ...
        if request.source_ip:
            ...
            recent = [
                ts
                for ts in self.rate_limits[request.source_ip]
...
class WebSocketProtector(InterfaceProtector):
    ...
    async def protect(self, request: InterfaceRequest) -> InterfaceResponse:
        ...
        if request.source_ip:
            ...
            recent = [
                ts
                for ts in self.message_rates[request.source_ip]
        ...
        if threat_score >= 40:
            ...
            return InterfaceResponse(
                action=ProtectionAction.ALLOW, reason="WebSocket allowed", threat_score=threat_score
            )
...
class RawSocketProtector(InterfaceProtector):
    ...
    async def protect(self, request: InterfaceRequest) -> InterfaceResponse:
        ...
        if threat_score >= 50:
            ...
            return InterfaceResponse(
                action=ProtectionAction.ALLOW, reason="Socket allowed", threat_score=threat_score
            )
    ...
    def learn_from_attack(self, request: InterfaceRequest, attack_type: str):
        ...
        if "destination_port" in request.metadata:
            ...
            logger.warning(
                f"🧠 Socket: Learned suspicious port {request.metadata['destination_port']}"
            )
...
def protect_websocket(func: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs):
        request = InterfaceRequest(interface_type="websocket", body=kwargs.get("message"))
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        print(
            f"Test 2 - XSS Attack: {response2.action.value} - {response2.reason} (Threat: {response2.threat_score:.1f})"
        )
        ...
        print(
            f"Test 3 - SQL Injection: {response3.action.value} - {response3.reason} (Threat: {response3.threat_score:.1f})"
        )
        ...
        print("Test 4 - Port Scan: Detected after scanning 30 ports")

        # Test 5: WebSocket Flooding
        for i in range(150):
            request = InterfaceRequest(
                interface_type="websocket", source_ip="198.51.100.52", body=b"flood message"
            )
            await protection.protect_request(request)

        print("Test 5 - WebSocket Flood: Detected after 150 messages")
