...
class AnomalyDetector:
    ...
    def record_metric(self, metric_name: str, value: float):
        ...
        self.metrics_history[metric_name].append({"value": value, "timestamp": datetime.utcnow()})
    ...
    def calculate_baseline(self, metric_name: str) -> Optional[Dict[str, float]]:
        ...
        return {"mean": mean, "std_dev": std_dev, "min": min(values), "max": max(values)}

    def detect_anomaly(
        self, metric_name: str, value: float, std_threshold: float = 3.0
    ) -> Tuple[bool, float]:
        """
