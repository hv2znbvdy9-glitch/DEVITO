...
# Passphrase for local log encryption – read from environment variable.
# Set AVA_ENCRYPTION_PASSPHRASE before running this script, e.g.:
#   $env:AVA_ENCRYPTION_PASSPHRASE = 'your-secret-passphrase'
# Alternatively, use Windows Credential Manager / DPAPI in production.
if ([string]::IsNullOrWhiteSpace($env:AVA_ENCRYPTION_PASSPHRASE)) {
    throw "Environment variable AVA_ENCRYPTION_PASSPHRASE is not set. Set it before running this script."
}
$script:EncryptionPassphrase = $env:AVA_ENCRYPTION_PASSPHRASE
...
$script:SmtpUser    = if ($env:AVA_SMTP_USER) { $env:AVA_SMTP_USER } else { '' }
$script:SmtpPass    = if ($env:AVA_SMTP_PASS) { $env:AVA_SMTP_PASS } else { '' }
