...
$script:EventFile = Join-Path $script:Logs 'events.jsonl'
$script:AlertFile = Join-Path $script:Logs 'alerts.jsonl'
...
function Add-AVATangle {
    ...
    $defaultState = [pscustomobject][ordered]@{ Cycle = 0; LastHash = '' }
...
function Invoke-AVACycle {
    ...
    foreach ($finding in @($analysis.Findings)) {
        Add-AVAJsonLine -Path $script:AlertFile -Data ([pscustomobject][ordered]@{
            Type = 'ALERT'
            TimestampUtc = Get-AVAUtc
            Cycle = $tangle.Cycle
            MainHash = $tangle.MainHash
...
function Install-AVALocalTask {
    ...
    if ($PSCmdlet.ShouldProcess($script:TaskName, 'Create local startup task')) {
        $argument = "-NoProfile -File `"$PSCommandPath`" -Mode Loop -IntervalSeconds 60"
        $action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument $argument
        $trigger = New-ScheduledTaskTrigger -AtStartup
        $taskPrincipal = New-ScheduledTaskPrincipal -UserId 'SYSTEM' -RunLevel Highest
        Register-ScheduledTask -TaskName $script:TaskName -Action $action -Trigger $trigger -Principal $taskPrincipal -Force | Out-Null
...
function Uninstall-AVALocalTask {
    ...
    if ($PSCmdlet.ShouldProcess($script:TaskName, 'Remove local startup task')) {
